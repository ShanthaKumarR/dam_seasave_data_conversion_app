...
class QFlagWindow(QDialog):
    ...
    def __init__(self, index_writer:Callable, file_name, output_file_name, string_check, data_writer, isBatch_file = False,  attribute_list= None):
        ...
        self.excluded_flags = ['timeS', 'scan', 'latitude', 'longitude', 'Bottle Fired']
        self.custom_flags = self.remove_attribute()
        self.Qflag_labels = [QLabel(self.first_frame ) for _ in range(len(self.custom_flags))]
        [my_lb.setObjectName(u"self.lb_"+str(i)) for i, my_lb in  zip(range(len(self.Qflag_labels)), self.Qflag_labels)]
        [label.setText(attribute) for label, attribute in zip(self.Qflag_labels, self.custom_flags)]
        self.Qflag_spbox = [QSpinBox(self.first_frame ) for _ in range(len(self.custom_flags))]
        self.Qflag_checkbox = [QCheckBox(self.first_frame ) for _ in range(len(self.custom_flags))]
        [my_chb.setObjectName(u"self.chb_"+str(i)) for i, my_chb in  zip(range(len(self.Qflag_checkbox)), self.Qflag_checkbox)]
        
        for i, l, cb, ch in zip(range(len(self.custom_flags)), self.Qflag_labels, self.Qflag_spbox, self.Qflag_checkbox):
        ...
        self.control_button.rejected.connect(self.close)
    ...
    def remove_attribute(self):
        ...
        for item in self.attribute_list:
            if item not in self.excluded_flags:
